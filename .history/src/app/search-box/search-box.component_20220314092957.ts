...
export class SearchBoxComponent implements OnInit {
  ...
  aplicaciones: ServidorBasicInfo[]
  loading = true
  selectedItems: ServidorBasicInfo;
  dropdownSettings = {};
  closeDropdownSelection = false;
  @Output() event = new EventEmitter<ServidorBasicInfo>()
  @Input() text: string
  ...
  getAplicaciones(){
    return this.http.get<ServidorBasicInfo[]>(`${this.url}/api/catalogos/aplicaciones`).subscribe(
      (response) => {
      this.loading = false;
