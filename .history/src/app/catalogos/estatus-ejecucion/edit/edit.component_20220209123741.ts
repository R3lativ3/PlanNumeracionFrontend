...
export class EditComponent implements OnInit {
  ...
  index = '/catalogos/empresas/'
  id : number
  model: empresa
  base: empresa

  constructor( private route: ActivatedRoute, private backend: EmpresaService, private router: Router) { }
  ...
  modifiedValues(base: empresa, model: empresa): string{
    let newValueColor = 'color:#82a951'
    let text = ''

    if(base.nombreEmpresa !== model.nombreEmpresa){
      text += `${this.base.nombreEmpresa} -> <strong style="${newValueColor}">${this.model.nombreEmpresa}</strong> <br/>`
    }
    text += '<br>'
