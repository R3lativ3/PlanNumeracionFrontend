...
export class ActividadesService {
  ...
  public createEjecucionActividad(body: ejecucionActividadPost[]){  
    ...
    for(let i = 0; i< body.length; i++){
      ...
      arr.append('item',formData)
    }*/
    let httpHeaders = new HttpHeaders({
      'Content-Type' : 'application/json'
    });
    const bodyx=JSON.stringify(body)
    console.log(bodyx)
    return this.http.post<Response>(`${this.url}/api/actividades/ejecuciones-actividad`, bodyx, {headers: httpHeaders})
  }
