...
export class ServidoresService {
  ...
  public getAdvancedSearchServidorXLS(servidor: servidor2){
    let httpHeaders = new HttpHeaders({'Content-Type' : 'application/json', 'Response-Type': 'application/octet-stream'});
    const body=JSON.stringify(servidor)
    const options = {headers: httpHeaders, params: body, responseType: 'blob'};
    return this.http.post<Blob>(`${this.url}/api/servidores/servidor/advanced-search/xls`, options) 
  }
