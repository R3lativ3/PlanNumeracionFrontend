...
export class ServidoresService {
  ...
  public advancedSearchServidor(servidor: servidor2, page?: number){
    ...
    return this.http.post<PaginatedServidor>(`${this.url}/api/servidores/servidor/advanced-search/${page}`, body, { headers: httpHeaders, responseType?: 'arraybuffer' as 'arr' }) 
  }

  public getAdvancedSearchServidorXLS(servidor: servidor2){
    let httpHeaders = new HttpHeaders({'Content-Type' : 'application/json', 'response-type': 'blob'});
    const body=JSON.stringify(servidor)
    return this.http.post<Blob>(`${this.url}/api/servidores/servidor/advanced-search/xls`, body, { headers: httpHeaders }) 
  }
