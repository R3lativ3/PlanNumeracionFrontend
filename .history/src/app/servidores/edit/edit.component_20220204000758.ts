...
export class EditComponent implements OnInit {
  ...
  servFijo: servidor // interfaz para comparar que cambio  
  hola: number
  ambientes: ambiente[]
  roles: rol[]
  ...
  ngOnInit(): void {
    ...
    this.backend.getServidorPostDto(1796).subscribe( data => {
      this.serv = data
      this.servFijo = data
      this.s = Object.assign({}, this.servFijo)
    })
  }
  ...
  modifiedValues(base: servidor, model: servidor): string{
    ...
    if(base.idVersionSistemaOperativo !== model.idVersionSistemaOperativo)    
      text += `${base.idVersionSistemaOperativo} - <strong>${model.idVersionSistemaOperativo}</strong> <br/>`
