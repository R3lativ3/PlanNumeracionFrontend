...
export class EditComponent implements OnInit {
  ...
  modifiedValues(base: servidor, model: servidor): string{
    ...
    if(base.idVersionSistemaOperativo !== model.idVersionSistemaOperativo)    
      text += `${ this.versionesSistemaOperativo.filter(x => x.id == base.idVersionSistemaOperativo).map(z => z.version) } - <strong>${ this.versionesSistemaOperativo.filter(x => x.id == model.idVersionSistemaOperativo).map(z => z.version) }</strong> <br/>`
