...
export class EditComponent implements OnInit {
  ...
  modifiedValues(base: servidor, model: servidor): string{
    ...
    if(base.idModeloHW !== model.idModeloHW) 
      text += "<li>"
      text += "<ul>"                                 
      text += `${this.modelosHW.filter(x => x.id == base.idModeloHW).map(z => z.modelo)}</ul> <ul><strong style="${newValueColor}">${this.modelosHW.filter(x => x.id == model.idModeloHW).map(z => z.modelo)}</strong></ul>`
      text += "</li>"
    ...
    if(base.idEmpresa !== model.idEmpresa) 
      text += `${this.empresas.filter(x => x.id == base.idEmpresa).map(z => z.nombreEmpresa)} -> <strong>${this.empresas.filter(x => x.id == model.idEmpresa).map(z => z.nombreEmpresa)}</strong> <br/>`
    
    if(base.idAmbiente !== model.idAmbiente) 
      text += `${this.ambientes.filter(x => x.id == base.idAmbiente).map(z => z.tipoAmbiente)} -> <strong>${this.ambientes.filter(x => x.id == model.idAmbiente).map(z => z.tipoAmbiente)}</strong> <br/>`
