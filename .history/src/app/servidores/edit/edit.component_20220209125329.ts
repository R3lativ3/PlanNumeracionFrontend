...
export class EditComponent implements OnInit {
  ...
  constructor( private route: ActivatedRoute, private backend: ServidoresService) { }
  ...
  update(){
    let text = this.modifiedValues(this.servFijo, this.serv)
    if(text.charAt(0) !== '<'){
      Swal.fire({
  ...
  getServidorInfo(id: number){
    this.backend.getServidorPostDto(id).subscribe( data => {
      this.serv = data
