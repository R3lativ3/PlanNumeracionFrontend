...
export class AdvancedSearchComponent implements OnInit {
  ...
  textSearch(model: servidor2): string{
    ...
    if( model.idAmbiente ) text += `Ambiente: ${this.ambientes.filter(x => model.idAmbiente?.includes(x.id)).map(z => z.tipoAmbiente)}, `
    /*
...
*/
  ...
  receiveSelectBox(selectList: number[]){
    console.log(selectList)
    this.serv.idAmbiente = selectList
  }
