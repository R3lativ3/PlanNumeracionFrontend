...
export class ServidoresService {
  ...
  public searchServidor(property?: string, page?: number, option?: number){
    if(property && page && option){
      if(option == 1){
        if(property!.indexOf(",") > -1){
          return this.http.get<PaginatedServidor>(`${this.url}/api/servidores/servidor/property?page=${page}&hostnameList=${property}`) 
        }
        return this.http.get<PaginatedServidor>(`${this.url}/api/servidores/servidor/property?page=${page}&hostname=${property}`) 
      }
  
      if(option == 2){
        if(property!.indexOf(",") > -1){
          return this.http.get<PaginatedServidor>(`${this.url}/api/servidores/servidor/property?page=${page}&ipList=${property}`) 
        }
        return this.http.get<PaginatedServidor>(`${this.url}/api/servidores/servidor/property?page=${page}&ip=${property}`) 
      }
    }
    return;
  }
  ...
  public getAdvancedSearchServidorXLS(servidor: servidor2){
    return this.http.post(`${this.url}/api/servidores/servidor/advanced-search/xls`, servidor, {
